...
def _(np, path_models, stats):
    ...
    return M, dict_model_data, dict_model_states
...
def _():
    ...
    exp_ids
    return dict_model_stat_units, exp_ids
...
def _(
    ...
    for hz, df_hz in df_data.groupby("Hazard Rate"):
        dict_model_change_order_dfs = {}
        dict_model_order = {}
        for _exp_id, states_first_M_zscore in dict_model_first_states_all.items():
            states_win, sample_win, target_win, order = get_ordered_sliding_window(
                states_first_M_zscore,
                samples[:, :M],
                targets[:, :M],
                df_hz,
                T,
            ...
            dict_model_order[_exp_id] = order
            _model_stat_units = dict_model_stat_units_all["hz"][_exp_id]
            stat_units = list(_model_stat_units.keys())
            ...
            for i, unit in enumerate(stat_units):
                ...
                df_unit = df_unit.assign(**{
                    "unit": unit,
                    "order": [f"Change {i+1}" for i in order],
                })

                unit_dfs.append(df_unit)

            dict_model_change_order_dfs[_exp_id] = pd.concat(unit_dfs).melt(
                id_vars=[
                    'unit',
                    'order',
                ],
        ...
        dict_hz_model_change_order_dfs[hz] = (dict_model_change_order_dfs, dict_model_order)
        ...
        # ])
    return T, change_idx, color_change_index, dict_hz_model_change_order_dfs
...
def _(
    ...
    for _exp_id, _path_model in path_models_all.items():
        ...
        dict_model_states[_exp_id] = _model_data["states"] #[np.arange(8192), -M:df_data["length"].values - 1]
    ...
    return (dict_model_first_states_all,)
...
def _(
    ...
    def get_activity_difference_around_criterion(states, samples, targets, df_selected, tau, k):
        ...
        criterion_mask = (targets[:, :, -k] == 1) # & (samples[:, :, 2:] != 127).any(axis=-1)
    ...
    for _exp_id, _states_first_M_zscore in dict_model_first_states_all.items():
        ...
        diff_states_no_change, _ = get_activity_difference_during_zero_criterion(
            _states_first_M_zscore,
            # samples[:, :M],
            targets[:, :M],
            df_data,
        ...
        dict_model_change_state_diffs[_exp_id] = {
            "hidden" if i == 0 else "cell": {
                ...
                "decay": np.abs(diff_states_no_change[:, unit]).mean(),
            }
            for i, unit in enumerate(_stat_units)
        }
    return (dict_model_change_state_diffs,)
...
@app.cell
def _(dict_model_change_state_diffs):
    dict_model_change_state_diffs
    return
...
@app.cell
def _(dict_model_change_state_diffs, plt):
    plt.hist(dict_model_change_state_diffs["san-4604"][15])
    plt.show()
    return


@app.cell
def _(model_stat_units):
    list(model_stat_units.keys())[0]
    return
